...
def general_dayinfo_pd(id):
    ...
    general_dayinfo_np = np.concatenate(
        (general_dayinfo_np, np.array([list(general.values())])))
    general_dayinfo_pd = pd.DataFrame(
        general_dayinfo_np, columns=list(
            general.keys()))
...
def dayprice_pd(id):
    ...
    for i in day:
        dayprice_np = np.concatenate(
            (dayprice_np, np.array([list(i.values())])))
    ...
    for i in range(len(dayprice_pd.time)):
        ...
        dayprice_pd.time[i] = pd.datetime(
            now.year, now.month, now.day, int(
                s[0]), int(
                s[1]))
...
def dayinfo_pd(id):
    ...
    dayinfo_np = np.concatenate(
        (dayinfo_np, np.array([list(day[0].values())])))
...
def history_pd(id):
    ...
    TP = (history_pd['max_price'] + history_pd['min_price'] +
          history_pd['close_price']) / 3
    n = 20
    constant = 0.015
...
def client_history_pd(id):
    ...
    for i in client_hist:
        client_history_np = np.concatenate(
            (client_history_np, np.array([list(i.values())])))
    client_history_pd = pd.DataFrame(
        client_history_np, columns=list(
            client_hist[1].keys()))
