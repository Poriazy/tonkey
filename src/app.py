...
@app.route("/")
def hello():
    market = data.market_pd
    return render_template(
        "index.html",
        market=market,
        strix='Flask Bootstrap Table')
...
def stosk_view(id):
    ...
    day_price_list = data.dayprice_pd(int(id))
    general = data.general_dayinfo_pd(id)
    saf = data.saf_pd(id)
    return render_template(
        "stock.html",
        id=id,
...
def client_history_view(id):
    hist = data.client_history_pd(int(id))
    market = data.market_pd
    symbol = market.loc[market['id'] == str(id)]['symbol'].values[0]
    name = market.loc[market['id'] == str(id)]['name'].values[0]
    return render_template(
