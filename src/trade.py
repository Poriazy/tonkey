            ...
            if(gap_p > 0):
                ...
                if (gap_calc > 0):
                    ...
                    if(rootdata["btc1h"] > (-0.5)):
                        ...
                        req = requests.get(
                            'http://bit.tyto.ir/change/buy/' + str(price))
                        ans.append(req)
    ...
    if (len(myorders) > 0):
        ...
        if (myorders[0]['side'] == 'sell'):
            ...
            if(sec_asks - 100000 > myorders[0]['price']):
                price = max(
                    sec_asks - 50000, int((mytrades['data'][0]['price'] * 1.01) / 50000 + 1) * 50000)
                req = requests.get('http://bit.tyto.ir/change/delbtc')
                ans.append(req)
                req = requests.get(
                    'http://bit.tyto.ir/change/sell/' + str(price))
                ans.append(req)
